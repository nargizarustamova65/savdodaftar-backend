...
class BillingService
{
    ...
    public function checkout(User $user, string $provider, string $plan): Payment
    {
        ...
        return DB::transaction(function () use ($payment, $reason, $cancelTime) {
            ...
            if ($newState === -2 && $payment->subscription_id !== null) {
                ...
                if ($subscription?->isActive()) {
                    ...
                    $subscription->update($expiresAt->isPast() ? ['status' => Subscription::STATUS_CANCELED, 'expires_at' => now()] : ['expires_at' => $expiresAt]);
                }
            }
            $payment->update(['status' => Payment::STATUS_CANCELED, 'canceled_at' => now(), 'provider_state' => $newState, 'meta' => array_merge($payment->meta ?? [], ['cancel_time' => $cancelTime, 'cancel_reason' => $reason])]);
            return $payment;
        });
    }
    ...
    {
        if ($payment->isPending()) $payment->update(['status' => Payment::STATUS_FAILED, 'meta' => array_merge($payment->meta ?? [], array_filter(['fail_note' => $note]))]);
        return $payment;
    }
